  ...
  <div class="flex w-full flex-wrap lg:pl-8" v-if="validator">
    <div class="w-full">
      <ul class="flex mb-0 list-none flex-wrap py-2 flex-row">
        ...
        <li class="-mb-px lg:mr-2 last:mr-0 ss:mb-2 flex-auto ml-1 lg:w-max text-center">
          <a class="text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal" v-on:click="toggleTabs(3)" v-bind:class="{ 'bg-gray-100 hover:bg-gray-200 dark:bg-oswapDark-gray dark:hover:bg-gray-900 ': openTab !== 3, 'text-white bg-oswapGreen': openTab === 3 }"> Ratio Panel </a>
        </li>
      </ul>
        ...
        <div class="p-3 flex-auto">
          <div class="tab-content tab-space">
            <div v-bind:class="{ 'hidden h-full': openTab !== 1, 'h-full flex flex-wrap justify-between': openTab === 1 }">
              <h5 class="w-full">Stake Content</h5>
              <input type="text" class="ring-black mt-2 ring-opacity-10 focus:ring-oswapGreen ring-1 focus:outline-none rounded-xl flex ss:w-full lg:w-9/12 z-20 bg-gray-100 py-2 items-center pl-3 dark:bg-oswapDark-gray dark:placeholder-gray-600 placeholder-gray-300" placeholder="Staked..." />
              <button class="flex w-full mt-2 lg:w-2/12 items-center justify-center rounded-xl border border-oswapGreen st5 text-oswapGreen hover:bg-oswapGreen hover:text-gray-200 dark:hover:text-slightDark">
                <p class="text-sm p-1 px-3">Stake</p>
              </button>
            </div>
            <div v-bind:class="{ 'hidden h-full': openTab !== 2, 'h-full flex flex-wrap justify-between': openTab === 2 }">
              <h5 class="w-full">Unstake Content</h5>
              <input type="text" class="ring-black mt-2 ring-opacity-10 focus:ring-oswapGreen ring-1 focus:outline-none rounded-xl flex ss:w-full lg:w-9/12 z-20 bg-gray-100 py-2 items-center pl-3 dark:bg-oswapDark-gray dark:placeholder-gray-600 placeholder-gray-300" placeholder="Staked..." />
              <button class="flex w-full lg:w-2/12 mt-2 items-center justify-center rounded-xl border border-oswapGreen st5 text-oswapGreen hover:bg-oswapGreen hover:text-gray-200 dark:hover:text-slightDark">
            ...
            <div v-bind:class="{ 'hidden h-full': openTab !== 3, 'h-full flex flex-wrap justify-between': openTab === 3 }">
              <h5 class="w-full">Ratio Content</h5>
            </div>
          </div>
        </div>
      </div>
